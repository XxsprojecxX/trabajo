...
def recolectar_instagram_con_proxy(post_url, instagram_username):
    ...
    enrutado a través del proxy de Dataimpulse.
    """
    # --- Carga Segura de Credenciales del Proxy ---
    load_dotenv()
    ...
    # --- Configuración de Instaloader y Proxy ---
    print("Iniciando instancia de Instaloader...")
    L = instaloader.Instaloader()

    print("Configurando proxy de Dataimpulse...")
    proxy = f"http://{DI_USERNAME}:{DI_PASSWORD}@{DI_HOST}:{DI_PORT}"
    L.context.proxies = {
        'http': proxy,
        'https': proxy
    }
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Uso: python recolector_final_v6.0.py <TU_USUARIO_DE_INSTAGRAM>")
        sys.exit(1)
    
    instagram_user = sys.argv[1]
    
    print("--- Iniciando Recolector v6.0 (Arquitectura Final) ---")
